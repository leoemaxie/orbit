...
<div class="space-y-5">
	<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
		...
		<Button variant="secondary" size="md" onclick={onRefresh} {loading} class="w-full sm:w-auto">
	...
	<div class="grid grid-cols-2 md:grid-cols-4 gap-3">
		<div class="bg-surface-900 border border-white/10 rounded-xl p-4 flex flex-col justify-between">
			...
			<div class="text-2xl font-bold font-mono text-slate-100 mt-2">{totalMissions}</div>
		</div>
		<div class="bg-surface-900 border border-white/10 rounded-xl p-4 flex flex-col justify-between">
