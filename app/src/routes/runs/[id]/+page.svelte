...
<div class="max-w-6xl mx-auto space-y-8">
	...
	{:else if run}
		...
		<div class="bg-surface-900 border border-white/10 rounded-xl p-5 space-y-4 shadow-2xl">
			<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/5 pb-4">
				<div>
					<div class="flex items-center gap-2 mb-1">
						...
						<code class="text-xs font-mono text-orbit-cyan">{run.id}</code>
				...
				<div class="flex items-center gap-2 flex-wrap text-xs font-mono">
					<div class="px-3 py-1 rounded-lg bg-surface-800 border border-white/5">
						<span class="text-slate-500">SOURCES:</span> <strong class="text-slate-200">{run.sources_found?.length || 0}</strong>
					</div>
					<div class="px-3 py-1 rounded-lg bg-surface-800 border border-white/5">
						<span class="text-slate-500">PAGES:</span> <strong class="text-slate-200">{run.pages_retrieved?.length || 0}</strong>
					</div>
					<div class="px-3 py-1 rounded-lg bg-surface-800 border border-white/5">
						<span class="text-slate-500">EXTRACTED:</span> <strong class="text-emerald-400">{run.extracted_count || 0}</strong>
					</div>
					<div class="px-3 py-1 rounded-lg bg-surface-800 border border-white/5">
						<span class="text-slate-500">VALIDATED:</span> <strong class="text-orbit-cyan">{run.validated_count || 0}</strong>
					</div>
				</div>
			</div>
