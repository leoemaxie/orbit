...
class WorkflowService:
    ...
    def get_adapter_topology(cls) -> list[dict[str, Any]]:
        ...
        topology = [
            ...
            {
                "id": "7", "label": "Cloud Object Storage", "category": "storage", "mode": "custom",
                "engine": "GCS / S3-Compatible Object Store", "iconName": "Cloud",
                "description": "Upload JSON artifacts and compiled dossiers to cloud buckets",
                "status": "active" if bool(s.storage_access_key or s.storage_backend in ("gcs", "s3")) else "optional",
                "config": {
                    "bucket_name": s.storage_bucket_name if (s.storage_access_key or s.storage_backend in ("gcs", "s3")) else "",
                    "region": s.storage_region if (s.storage_access_key or s.storage_backend in ("gcs", "s3")) else "",
                    "endpoint_url": s.storage_endpoint_url or "",
                    "access_key": SecretVault.mask_secret(s.storage_access_key),
                    "secret_key": SecretVault.mask_secret(s.storage_secret_key),
                },
            },
            {
